...
async function testPatientManagement() {
  ...
  try {
    ...
    const createResponse = await apiClient.post('/dental/patients', testPatient);
    ...
    const getResponse = await apiClient.get(`/dental/patients/${createdPatientId}`);
    log.success(`Retrieved patient: ${getResponse.data.data.name}`);
    
    // Update patient
    log.info('Updating test patient...');
    const updateResponse = await apiClient.put(`/dental/patients/${createdPatientId}`, {
      ...testPatient,
...
async function testAppointmentManagement() {
  ...
  try {
    ...
    const createResponse = await apiClient.post('/dental/appointments', appointmentData);
    ...
    const dateResponse = await apiClient.get(`/dental/appointments/date/${testAppointment.date}`);
    log.success(`Retrieved ${dateResponse.data.data.length} appointments for the date`);
    
    // Update appointment
    log.info('Updating test appointment...');
    const updateResponse = await apiClient.put(`/dental/appointments/${createdAppointmentId}`, {
      ...appointmentData,
      status: 'confirmed'
    });
...
async function testMedicalRecords() {
  ...
  try {
    ...
    const createResponse = await apiClient.post(`/dental/patients/${createdPatientId}/records`, testMedicalRecord);
    ...
    const listResponse = await apiClient.get(`/dental/patients/${createdPatientId}/records`);
    log.success(`Retrieved ${listResponse.data.data.length} medical records`);
    
    // Update medical record
    log.info('Updating test medical record...');
    const updateResponse = await apiClient.put(`/dental/records/${createdMedicalRecordId}`, {
      ...testMedicalRecord,
...
async function testInventoryManagement() {
  ...
  try {
    ...
    log.success(`Inbound record created with ID: ${inboundResponse.data.data.id}`);
    ...
    log.success(`Outbound record created with ID: ${outboundResponse.data.data.id}`);
...
async function runTests() {
  ...
  try {
    ...
    log.info(`Appointment Management: ${appointmentResult ? 'PASSED' : 'FAILED'}`);
    log.info(`Medical Records: ${medicalRecordResult ? 'PASSED' : 'FAILED'}`);
    log.info(`Inventory Management: ${inventoryResult ? 'PASSED' : 'FAILED'}`);
    
    const allPassed = authResult && patientResult && appointmentResult && 
                      medicalRecordResult && inventoryResult;
