...
async function testInventoryApi() {
  ...
  try {
    ...
    if (
      ...
      inventoryResponse.data.code === 200 &&
  ...
  if (testInventoryId) {
    try {
      ...
      const detailResponse = await axios.get(
        `${baseUrl}/dental/inventory/${testInventoryId}`,
        { headers },
      );

      if (
        detailResponse.data &&
        detailResponse.data.code === 200 &&
        detailResponse.data.data
      ) {
        console.log('✅ 成功获取库存详情');
        console.log(`   库存名称: ${detailResponse.data.data.name}`);
        console.log(
          `   当前库存: ${detailResponse.data.data.currentQuantity} ${detailResponse.data.data.unit}`,
        );
      } else {
  ...
  if (testInventoryId) {
    try {
      ...
      if (
        createInRecordResponse.data &&
        createInRecordResponse.data.code === 200
      ) {
        console.log('✅ 成功创建入库记录');

        // 获取入库记录详情
        if (
          createInRecordResponse.data.data &&
          createInRecordResponse.data.data.id
        ) {
          ...
          try {
            ...
            if (
              inRecordDetailResponse.data &&
              inRecordDetailResponse.data.code === 200 &&
              inRecordDetailResponse.data.data
            ) {
              console.log('✅ 成功获取入库记录详情');
              console.log(
                `   入库数量: ${inRecordDetailResponse.data.data.quantity}`,
              );
              console.log(
                `   单价: ${inRecordDetailResponse.data.data.unitPrice}`,
              );
            } else {
              console.log('❌ 获取入库记录详情失败');
              console.log(
                '返回内容:',
                JSON.stringify(inRecordDetailResponse.data, null, 2),
              );
            }
          } catch (error) {
        ...
        console.log(
          '返回内容:',
          JSON.stringify(createInRecordResponse.data, null, 2),
        );
      }
    } catch (error) {
  ...
  if (testInventoryId) {
    try {
      ...
      if (
        createOutRecordResponse.data &&
        createOutRecordResponse.data.code === 200
      ) {
        console.log('✅ 成功创建出库记录');

        // 获取出库记录详情
        if (
          createOutRecordResponse.data.data &&
          createOutRecordResponse.data.data.id
        ) {
          ...
          try {
            ...
            if (
              outRecordDetailResponse.data &&
              outRecordDetailResponse.data.code === 200 &&
              outRecordDetailResponse.data.data
            ) {
              console.log('✅ 成功获取出库记录详情');
              console.log(
                `   出库数量: ${outRecordDetailResponse.data.data.quantity}`,
              );
              console.log(
                `   领用人: ${outRecordDetailResponse.data.data.recipient}`,
              );
            } else {
              console.log('❌ 获取出库记录详情失败');
              console.log(
                '返回内容:',
                JSON.stringify(outRecordDetailResponse.data, null, 2),
              );
            }
          } catch (error) {
        ...
        console.log(
          '返回内容:',
          JSON.stringify(createOutRecordResponse.data, null, 2),
        );
      }
    } catch (error) {
  ...
  try {
    console.log('\n9. 获取库存统计信息...');
    console.log(
      '⚠️ 当前版本暂不支持通过API直接获取库存统计信息，跳过此测试用例',
    );
    /*
  ...
  try {
    console.log('\n10. 获取入库记录列表...');
    const inRecordsResponse = await axios.get(
      `${baseUrl}/dental/inventory/records/in/list`,
      { headers },
    );
  ...
  try {
    console.log('\n11. 获取出库记录列表...');
    const outRecordsResponse = await axios.get(
      `${baseUrl}/dental/inventory/records/out/list`,
      { headers },
    );
    ...
    if (outRecordsResponse.data && outRecordsResponse.data.code === 200) {
      ...
      console.log(
        '返回内容:',
        JSON.stringify(outRecordsResponse.data, null, 2),
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (
      ...
      const deletedItemExists = items.some(
      ...
      console.log(
        `   当前搜索到与"Test Item"相关的库存项: ${items.length} 条记录`,
      );
    } else {
  ...
  if (testInRecordId) {
    try {
      ...
      if (
        deleteInRecordResponse.data &&
        deleteInRecordResponse.data.code === 200
      ) {
        ...
        console.log(
          '返回内容:',
          JSON.stringify(deleteInRecordResponse.data, null, 2),
        );
      }
    } catch (error) {
  ...
  if (testOutRecordId) {
    try {
      ...
      if (
        deleteOutRecordResponse.data &&
        deleteOutRecordResponse.data.code === 200
      ) {
        ...
        console.log(
          '返回内容:',
          JSON.stringify(deleteOutRecordResponse.data, null, 2),
        );
      }
    } catch (error) {
...
async function testStockTakeApi() {
  ...
  try {
    ...
    if (
      ...
      if (itemCount > 0) {
        testInventoryId = inventoryResponse.data.data.items[0].id;
        console.log(`   使用库存项ID: ${testInventoryId} 进行盘点测试`);
      } else {
        // 如果没有库存，创建一个测试库存
        console.log('\n2.1. 未找到库存记录，创建测试库存项...');
        const testInventory = {
          name: 'Test Item For StockTake',
          code: 'STOCKTAKE001',
          type: 'material',
          specification: 'Test Spec',
          unit: 'pcs',
          ...
          manufacturer: 'Test Manufacturer',
          referencePrice: '10.00',
          remarks: 'Created for stock-take testing',
        };
  ...
  if (testInventoryId) {
    try {
      ...
      const stockTakeData = {
        ...
        stockTakeDate: new Date().toISOString().split('T')[0], // 今天的日期，格式为YYYY-MM-DD
        operator: '测试盘点员',
        remarks: '这是一个自动化测试生成的盘点记录',
        items: [
          {
            inventoryId: testInventoryId,
            actualQuantity: 95, // 故意与系统数量不同，以测试差异处理
            reason: '测试盘点差异',
          },
        ],
      };

      console.log('发送盘点数据:', JSON.stringify(stockTakeData, null, 2));
      const createStockTakeResponse = await axios.post(
        `${baseUrl}/dental/inventory/stock-takes`,
        stockTakeData,
        { headers },
      );

      if (
        createStockTakeResponse.data &&
        createStockTakeResponse.data.code === 200
      ) {
        console.log('✅ 成功创建库存盘点记录');

        // 获取盘点记录ID用于后续测试
        if (
          createStockTakeResponse.data.data &&
          createStockTakeResponse.data.data.id
        ) {
          testStockTakeId = createStockTakeResponse.data.data.id;
          console.log(`   盘点记录ID: ${testStockTakeId}`);
          console.log(
            `   盘点结果摘要: ${
              createStockTakeResponse.data.data.resultSummary || '无'
            }`,
          );
        }
      } else {
        console.log('❌ 创建库存盘点记录失败');
        console.log(
          '返回内容:',
          JSON.stringify(createStockTakeResponse.data, null, 2),
        );
      }
    } catch (error) {
      console.log('❌ 创建库存盘点记录接口测试失败');
      console.log('错误信息:', error.message);
      if (error.response) {
        console.log('响应状态:', error.response.status);
        console.log('响应数据:', JSON.stringify(error.response.data, null, 2));
    ...
    console.log('\n3. 跳过创建库存盘点记录测试，因为没有可用的库存项ID');
  }

  // 步骤4: 获取库存盘点记录列表
  try {
    console.log('\n4. 获取库存盘点记录列表...');
    const stockTakeListResponse = await axios.get(
      `${baseUrl}/dental/inventory/stock-takes`,
      {
        headers,
        params: {
    ...
    if (stockTakeListResponse.data && stockTakeListResponse.data.code === 200) {
      const itemCount = stockTakeListResponse.data.data?.items?.length || 0;
      console.log(`✅ 成功获取盘点记录列表，共 ${itemCount} 条记录`);
    } else {
      console.log('❌ 获取盘点记录列表失败');
      console.log(
        '返回内容:',
        JSON.stringify(stockTakeListResponse.data, null, 2),
      );
    }
  } catch (error) {
    console.log('❌ 获取盘点记录列表接口测试失败');
    console.log('错误信息:', error.message);
    if (error.response) {
      console.log('响应状态:', error.response.status);
      console.log('响应数据:', JSON.stringify(error.response.data, null, 2));
  ...
  if (testStockTakeId) {
    try {
      console.log('\n5. 获取盘点记录详情...');
      const stockTakeDetailResponse = await axios.get(
        `${baseUrl}/dental/inventory/stock-takes/${testStockTakeId}`,
        { headers },
      );

      if (
        stockTakeDetailResponse.data &&
        stockTakeDetailResponse.data.code === 200 &&
        stockTakeDetailResponse.data.data
      ) {
        console.log('✅ 成功获取盘点记录详情');
        console.log(
          `   批次号: ${stockTakeDetailResponse.data.data.batchNumber}`,
        );
        console.log(`   操作人: ${stockTakeDetailResponse.data.data.operator}`);
        console.log(
          `   盘点项数量: ${
            stockTakeDetailResponse.data.data.items?.length || 0
          }`,
        );
      } else {
        console.log('❌ 获取盘点记录详情失败');
        console.log(
          '返回内容:',
          JSON.stringify(stockTakeDetailResponse.data, null, 2),
        );
      }
    } catch (error) {
      console.log('❌ 获取盘点记录详情接口测试失败');
      console.log('错误信息:', error.message);
      if (error.response) {
        console.log('响应状态:', error.response.status);
        console.log('响应数据:', JSON.stringify(error.response.data, null, 2));
    ...
    console.log('\n5. 跳过获取盘点记录详情测试，因为没有可用的盘点记录ID');
  }

  // 步骤6: 验证库存数量是否已根据盘点结果更新
  if (testInventoryId) {
    try {
      console.log('\n6. 验证库存数量是否已根据盘点结果更新...');
      const inventoryDetailResponse = await axios.get(
        `${baseUrl}/dental/inventory/${testInventoryId}`,
        { headers },
      );

      if (
        inventoryDetailResponse.data &&
        inventoryDetailResponse.data.code === 200 &&
        inventoryDetailResponse.data.data
      ) {
        console.log('✅ 成功获取库存详情');
        console.log(
          `   当前库存: ${inventoryDetailResponse.data.data.currentQuantity} ${inventoryDetailResponse.data.data.unit}`,
        );
        console.log(
          `   验证结果: ${
            inventoryDetailResponse.data.data.currentQuantity === 95
              ? '盘点后数量已更新'
              : '盘点结果未应用'
          }`,
        );
        ...
        console.log(
          '返回内容:',
          JSON.stringify(inventoryDetailResponse.data, null, 2),
        );
      }
    } catch (error) {
      console.log('❌ 获取库存详情接口测试失败');
  ...
  if (testStockTakeId) {
    try {
      console.log('\n7. 删除盘点记录...');
      const deleteStockTakeResponse = await axios.delete(
        `${baseUrl}/dental/inventory/stock-takes/${testStockTakeId}`,
        { headers },
      );

      if (
        deleteStockTakeResponse.data &&
        deleteStockTakeResponse.data.code === 200
      ) {
        console.log('✅ 成功删除盘点记录');
      } else {
        console.log('❌ 删除盘点记录失败');
        console.log(
          '返回内容:',
          JSON.stringify(deleteStockTakeResponse.data, null, 2),
        );
      }
    } catch (error) {
      console.log('❌ 删除盘点记录接口测试失败');
      console.log('错误信息:', error.message);
      if (error.response) {
        console.log('响应状态:', error.response.status);
        console.log('响应数据:', JSON.stringify(error.response.data, null, 2));
...
// 运行库存盘点测试
testStockTakeApi().catch((error) => {
  console.error('盘点测试过程中出现未捕获的错误:', error);
});
