...
async function testAppointmentCreation() {
  ...
  try {
    ...
    const response = await apiClient.post('/dental/appointments', testAppointment);
...
async function testGetAppointments() {
  ...
  try {
    ...
    const dateResponse = await apiClient.get(`/dental/appointments/date/${tomorrowFormatted}`);
    
    const dateAppointments = dateResponse.data.data || [];
    log.success(`成功获取 ${tomorrowFormatted} 的预约，共 ${dateAppointments.length} 条`);
...
async function testGetAppointmentDetails() {
  ...
  try {
    ...
    const response = await apiClient.get(`/dental/appointments/${testAppointmentId}`);
    ...
    log.info(`预约时间: ${appointment.date} ${appointment.time}, 类型: ${appointment.type}`);
...
async function testUpdateAppointment() {
  ...
  try {
    ...
    const response = await apiClient.put(`/dental/appointments/${testAppointmentId}`, updatedAppointment);
    ...
    // 验证更新结果
    const verifyResponse = await apiClient.get(`/dental/appointments/${testAppointmentId}`);
    const updated = verifyResponse.data.data;
    
    log.info(`更新后的预约时间: ${updated.date} ${updated.time}, 持续时间: ${updated.duration}分钟`);
...
async function testCancelAppointment() {
  ...
  try {
    ...
    const response = await apiClient.put(`/dental/appointments/${testAppointmentId}`, {
      status: 'CANCELLED',
    ...
    log.info(`预约状态: ${appointment.status}, 取消原因: ${appointment.cancellationReason}`);
...
async function testDeleteAppointment() {
  ...
  try {
    ...
    const response = await apiClient.delete(`/dental/appointments/${testAppointmentId}`);
