...
async function runApiTests() {
  ...
  try {
    ...
    if (patientsResponse.data && patientsResponse.data.success) {
      ...
      console.log(`   获取到 ${patientsResponse.data.data?.items?.length || 0} 个患者记录`);
    } else {
  ...
  try {
    console.log('\n3. 测试预约列表接口...');
    const appointmentsResponse = await axios.get(`${baseUrl}/dental/appointments`, { headers });
    
    if (appointmentsResponse.data && appointmentsResponse.data.success) {
      console.log('✅ 成功获取预约列表');
      console.log(`   获取到 ${appointmentsResponse.data.data?.items?.length || 0} 个预约记录`);
    } else {
      console.log('❌ 预约列表接口返回了数据，但格式不符合预期');
      console.log('返回内容:', JSON.stringify(appointmentsResponse.data, null, 2));
    }
  } catch (error) {
  ...
  try {
    ...
    if (inventoryResponse.data && inventoryResponse.data.success) {
      ...
      console.log(`   获取到 ${inventoryResponse.data.data?.items?.length || 0} 个库存记录`);
    } else {
