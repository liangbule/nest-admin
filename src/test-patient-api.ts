...
async function testPatientApi() {
  ...
  try {
    ...
    const patientsResponse = await axios.get(`${baseUrl}/dental/patients`, { headers });
    console.log('患者列表响应:', JSON.stringify(patientsResponse.data, null, 2));
  ...
  try {
    ...
    const createResponse = await axios.post(`${baseUrl}/dental/patients`, testPatient, { headers });
  ...
  if (testPatientId) {
    try {
      ...
      const detailResponse = await axios.get(`${baseUrl}/dental/patients/${testPatientId}`, { headers });
      console.log('患者详情响应:', JSON.stringify(detailResponse.data, null, 2));
  ...
  if (testPatientId) {
    try {
      ...
      const updateData = {
        name: '更新后的测试患者_' + new Date().getTime().toString().substring(8),
      ...
      const updateResponse = await axios.put(`${baseUrl}/dental/patients/${testPatientId}`, updateData, { headers });
      console.log('更新患者响应:', JSON.stringify(updateResponse.data, null, 2));
  ...
  if (testPatientId) {
    try {
      ...
      const appointmentResponse = await axios.post(`${baseUrl}/dental/appointments`, appointmentData, { headers });
      console.log('创建预约响应:', JSON.stringify(appointmentResponse.data, null, 2));
      
      // 尝试多种响应格式
      const responseData = appointmentResponse.data?.data || appointmentResponse.data;
      const success = responseData?.success !== false;
  ...
  if (testPatientId) {
    try {
      ...
      console.log('请求URL:', `${baseUrl}/dental/patients/${testPatientId}/records`);
      console.log('发送的医疗记录数据:', JSON.stringify(recordData, null, 2));
      
      const recordResponse = await axios.post(`${baseUrl}/dental/patients/${testPatientId}/records`, recordData, { headers });
      console.log('创建医疗记录响应:', JSON.stringify(recordResponse.data, null, 2));
