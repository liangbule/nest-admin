...
export class FixMedicalRecordsColumnNames1744860000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      await queryRunner.query(`ALTER TABLE \`dental_medical_records\` DROP FOREIGN KEY IF EXISTS \`FK_dental_medical_records_patient\``);
      await queryRunner.query(`ALTER TABLE \`dental_followups\` DROP FOREIGN KEY IF EXISTS \`FK_dental_followups_medical_record\``);
    } catch (e) {
    ...
    const hasPatientId = await checkColumnExists('dental_medical_records', 'patientId');
    const hasPatient_id = await checkColumnExists('dental_medical_records', 'patient_id');
    
    if (!hasPatientId && hasPatient_id) {
      console.log('发现patient_id列，需要重命名为patientId...');
      await safeRenameColumn('dental_medical_records', 'patient_id', 'patientId', 'varchar(36) NULL');
    } else if (!hasPatientId && !hasPatient_id) {
      console.log('创建缺失的patientId列...');
      await safeAddColumn('dental_medical_records', 'patientId', 'varchar(36) NULL');
    }
    
    // 2. 添加主诉字段
    await safeAddColumn('dental_medical_records', 'chiefComplaint', 'text NULL');
    
    // 3. 添加主治医生字段
    await safeAddColumn('dental_medical_records', 'attendingDoctor', 'varchar(255) NULL');
    
    // 4. 尝试将现有数据映射到新字段
    const hasDentistName = await checkColumnExists('dental_medical_records', 'dentistName');
    const hasAttendingDoctor = await checkColumnExists('dental_medical_records', 'attendingDoctor');
    const hasSymptoms = await checkColumnExists('dental_medical_records', 'symptoms');
    const hasChiefComplaint = await checkColumnExists('dental_medical_records', 'chiefComplaint');
