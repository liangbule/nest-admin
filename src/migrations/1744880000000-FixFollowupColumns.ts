...
export class FixFollowupColumns1744880000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      const hasFollowupDate = await checkColumnExists(
        'dental_followups',
        'followupDate',
      );
      const hasScheduledDate = await checkColumnExists(
        'dental_followups',
        'scheduledDate',
      );
      const hasCompletedDate = await checkColumnExists(
        'dental_followups',
        'completedDate',
      );
