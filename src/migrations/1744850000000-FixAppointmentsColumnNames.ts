...
export class FixAppointmentsColumnNames1744850000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      await queryRunner.query(`ALTER TABLE \`dental_appointments\` DROP FOREIGN KEY IF EXISTS \`FK_dental_appointments_patient\``);
    } catch (e) {
    ...
    const hasPatientId = await checkColumnExists('dental_appointments', 'patientId');
    const hasPatient_id = await checkColumnExists('dental_appointments', 'patient_id');
    
    if (!hasPatientId && hasPatient_id) {
      console.log('发现patient_id列，需要重命名为patientId...');
      await safeRenameColumn('dental_appointments', 'patient_id', 'patientId', 'varchar(36) NULL');
    } else if (!hasPatientId && !hasPatient_id) {
      console.log('创建缺失的patientId列...');
      await safeAddColumn('dental_appointments', 'patientId', 'varchar(36) NULL');
    }
    
    // 2. 确保appointmentTime列存在
    const hasAppointmentTime = await checkColumnExists('dental_appointments', 'appointmentTime');
    ...
    try {
      ...
      console.error('添加外键约束失败，可能是patientId与患者表的id类型不匹配:', e);
    }
