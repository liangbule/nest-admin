...
export class FixInventoryReferencePrice1744892000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      const hasReferencePriceSnake = await this.columnExists(
        queryRunner,
        'dental_inventory',
        'reference_price',
      );
      // 检查referencePrice列是否存在
      const hasReferencePriceCamel = await this.columnExists(
        queryRunner,
        'dental_inventory',
        'referencePrice',
      );
      ...
      if (!hasReferencePriceSnake && !hasReferencePriceCamel) {
        ...
        await queryRunner.query(
          `ALTER TABLE dental_inventory ADD reference_price DECIMAL(10,2) NULL COMMENT '单价参考值'`,
        );
        console.log('✅ 创建了reference_price列');
      } else if (hasReferencePriceCamel && !hasReferencePriceSnake) {
        // 只有驼峰式存在，重命名为下划线式
        await queryRunner.query(
          `ALTER TABLE dental_inventory CHANGE referencePrice reference_price DECIMAL(10,2) NULL`,
        );
      ...
      const hasDeleteTimeSnake = await this.columnExists(
        queryRunner,
        'dental_inventory',
        'delete_time',
      );
      const hasDeleteTimeCamel = await this.columnExists(
        queryRunner,
        'dental_inventory',
        'deleteTime',
      );
      ...
      if (!hasDeleteTimeSnake && !hasDeleteTimeCamel) {
        ...
        await queryRunner.query(
          `ALTER TABLE dental_inventory ADD delete_time TIMESTAMP NULL COMMENT '删除时间'`,
        );
        console.log('✅ 创建了delete_time列');
      } else if (hasDeleteTimeCamel && !hasDeleteTimeSnake) {
        // 只有驼峰式存在，重命名为下划线式
        await queryRunner.query(
          `ALTER TABLE dental_inventory CHANGE deleteTime delete_time TIMESTAMP NULL`,
        );
