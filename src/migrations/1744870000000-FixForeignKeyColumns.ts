...
export class FixForeignKeyColumns1744870000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // 检查外键是否存在
    const checkForeignKeyExists = async (table: string, fkName: string): Promise<boolean> => {
    ...
    // 安全重命名列，处理外键
    const safeRenameColumnWithFK = async (table: string, oldName: string, newName: string, definition: string, refTable: string, refColumn: string): Promise<void> => {
      try {
        // 检查列是否存在
        ...
        if (!oldExists && !newExists) {
          ...
          await queryRunner.query(`ALTER TABLE \`${table}\` ADD \`${newName}\` ${definition}`);
          return;
        }
        ...
        if (oldExists && newExists) {
          ...
          `);
          // 最后删除旧列
          await queryRunner.query(`ALTER TABLE \`${table}\` DROP COLUMN \`${oldName}\``);
          return;
        }
        
        // 旧列存在，新列不存在，需要重命名
        // 先删除外键约束
        const constraintName = `FK_${table}_${refTable.replace('dental_', '')}_${oldName}`;
        ...
        // 重新添加外键约束
        if (fkExists) {
          console.log(`重新添加外键约束: ${table}.${newName} -> ${refTable}.${refColumn}`);
          await queryRunner.query(`
            ALTER TABLE \`${table}\` 
            ADD CONSTRAINT \`FK_${table}_${refTable.replace('dental_', '')}_${newName}\` 
